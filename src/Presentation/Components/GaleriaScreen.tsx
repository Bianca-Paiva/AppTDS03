...
export default function GaleriaScreen() {
    ...
    // Função independente para acessar a geleria de fotos do dispositivo (SmatPhone ou Tablet)
    const abrirGaleria = async () => {
        // Solicitar a permissão de acesso à biblioteca de mídia do dispositivo
        const dadosPermissao = await ImagePicker.requestMediaLibraryPermissionsAsync();

        //Se o usuário recusar a permissãol exibe um alerta e interromepe a funçao
        if (!dadosPermissao.granted) {
            if (Platform.OS === 'android') {
                ToastAndroid.show("Aviso!: Permissão da galeria negada!", ToastAndroid.LONG);
            } else {
                Alert.alert("Aviso!", "Precisamos de acesso às suas fotos para continuar.")
            }
            return;
        }
        ...
        const resultado = await ImagePicker.launchImageLibraryAsync({
            ...
            mediaTypes: ['images'],
            // Havukuta as ferramentas nativas para o usuário cortar/girar a foto
            allowsEditing: true,
            // Define a prporção do corte 'ex: 3x4 ou 1x1 para o quadrado'
            aspect: [4, 3],
